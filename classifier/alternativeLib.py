...
class LiberalIlliberalScorer:
    ...
    def score_liberal_illiberal(self, text, thr=0.15):
        ...
        top_liberal   = sorted([h for h in liberal_hyps   if h['score_impact'] >= 0.5], key=lambda x: x['score_impact'], reverse=True)[:10]
        top_illiberal = sorted([h for h in illiberal_hyps if h['score_impact'] >= 0.5], key=lambda x: x['score_impact'], reverse=True)[:10]
    ...
    def quick_score(self, text, thr=0.25):
