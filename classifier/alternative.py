...
class LeftRightEconomicScorer:
    ...
    def score_left_right(self, text, thr=0.15):
        ...
        top_left = sorted([h for h in left_hyps if h['score_impact'] > 0], key=lambda x: x['score_impact'], reverse=True)[:10]
        top_right = sorted([h for h in right_hyps if h['score_impact'] > 0], key=lambda x: x['score_impact'], reverse=True)[:10]
